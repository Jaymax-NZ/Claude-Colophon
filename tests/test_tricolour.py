"""Session membership is decided by the triple's contents, not by the path.
...
TRICOLOUR = ".identicon/repository-identicon.tricolour"
TRIPLE = "🟥🟫🟥\n"
OTHER = "🟦🟦🟧\n"
...
def make_repo(root, triple):
    (root / ".identicon").mkdir(parents=True, exist_ok=True)
    (root / TRICOLOUR).write_text(triple, encoding="utf-8")
    git(root, "init", "-q")
    git(root, "add", "-A")
...
class TestMembership(TricolourCase):
    ...
    def test_a_neighbour_with_a_different_triple_does_not_match(self):
