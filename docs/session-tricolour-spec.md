...
Prefix the name of every Claude session with the tricolour of the repository it
is working in, so that a list of sessions is readable without reading it.
...
Recovered on 2026-09-03 from `claude/session-tricolour-tagging-kzqtab`, a cloud
branch that was never merged. The implementation it specifies was left there:
`skills/tricolour/` on main does the same work with a 105-line helper and the
rules written in `SKILL.md`, rather than that branch's 501-line script. Read this
document for the decisions, not for the code. One phrase changed on import — "the
